...
suite('API/DECISIONS -> FR case -> simple GET-s', () => {
    test('GET non-existent state (/blahBlah) - error handling ', () => generateAPIRequestForFR('GET', '/blahBlah', {})
        .catch(error => {
    ...
    test('GET 1st step (/create)', () => generateAPIRequestForFR('GET', '/create', {})
        .then(response => {
    ...
    test('GET 2nd step (/reject-reasons)', () => generateAPIRequestForFR('GET', '/reject-reasons', {})
        .then(response => {
            response.statusCode.should.be.eql(200)
            response.body.should.have.property('meta')
            response.body.meta.should.have.property('idPrefix')
...
suite('API/DECISIONS -> any case -> testing of values persistency (generic, no specific case-type related)', () => {
    ...
    test('GET - should have empty formValues', () => generateAPIRequestForFR('GET', '/create', {})
        .then(response => {
            checkResponseIsOkAndHasFormValues(response)
            response.body.formValues.should.be.eql({})
        }))
    test('POST - set some formValues', () => generateAPIRequestForFR('POST', '/create', {
        body: {
    ...
    test('GET - should have formValues set in the previous one', () => generateAPIRequestForFR('GET', '/create', {})
        .then(response => {
            checkResponseIsOkAndHasFormValues(response)
            response.body.formValues.should.be.eql(testValuesObjectA)
        }))
...
suite('API/DECISIONS -> FR case -> Full scenario of user "APPROVE the draft"', () => {
    ...
    test('POST - approved the draft', () => generateAPIRequestForFR('POST', `/${nextStepId}`, {
        body: {
    ...
    test('POST - filled notes for court admin', () => generateAPIRequestForFR('POST', `/${nextStepId}`, {
        body: {
            formValues: { approveDraftConsent: 'yes' },
            event: 'continue'
        }
    ...
    test('POST - final submit', () => generateAPIRequestForFR('POST', `/${nextStepId}`, {
        body: {
            formValues: { approveDraftConsent: 'yes' },
            event: 'continue'
        }
...
suite('API/DECISIONS -> FR case -> Full scenario of user "REJECT the draft"', () => {
    ...
    test('POST - rejected the draft', () => generateAPIRequestForFR('POST', '/create', {
        body: {
    ...
    test('POST - rejected the draft, NO hearing needed, NO annotated version needed', () => generateAPIRequestForFR('POST', '/reject-reasons', {
        body: {
            formValues: { approveDraftConsent: 'no' },
            event: 'continue'
        }
    ...
    test('POST - rejected the draft, hearing needed, NO annotated version needed', () => generateAPIRequestForFR('POST', '/reject-reasons', {
        body: {
    ...
    test('POST - rejected the draft, annotated version needed', () => generateAPIRequestForFR('POST', '/reject-reasons', {
        body: {
    ...
    test('POST - filled annotation, after rejecting the draft, now looking for hearing', () => generateAPIRequestForFR('POST', '/draft-consent-order', {
        body: {
            formValues: { partiesNeedAttend: true },
            event: 'continue'
        }
    ...
    test('POST - filled annotation, after rejecting the draft, hearing NOT needed', () => generateAPIRequestForFR('POST', '/draft-consent-order', {
        body: {
