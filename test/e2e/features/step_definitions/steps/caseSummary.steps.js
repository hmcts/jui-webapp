...
defineSupportCode(function({ Given, When, Then }) {
    ...
    Then(/^I should see linked cases or panel members details for that case(.*)$/, async function(type) {
        ...
        {
            await expect(caseSummaryPage.caseDetails_header_text.get(1).isDisplayed()).to.eventually.be.true;
            await expect(caseSummaryPage.caseDetails_header_text.get(1).getText()).to.eventually.equal("Representatives");
            await expect(caseSummaryPage.caseDetails_header_text.get(2).isDisplayed()).to.eventually.be.true;
            await expect(caseSummaryPage.caseDetails_header_text.get(2).getText()).to.eventually.equal("Linked cases");
