...
// const { setCookie } = require('api/auth/getTokenFromCode');
// const cookie;
// const proxy = require('api/lib/proxy');
const config = require('config/index.js');
const secret = process.env.IDAM_SECRET || 'AAAAAAAAAAAAAAAA';
const idam_client = 'juiwebapp';

console.log(secret);
const buffer = Buffer(`${idam_client}:${secret}`).toString('base64');

console.log(buffer);

const Authorization = `Basic ${buffer}`;
 // const Authorization = `Basic ${new Buffer(`juiwebapp :${secret}`).toString('base64')}`;
